...
public class Map1 : MonoBehaviour {
	...
	void Update () 
    {
        if (!GM.GameManager.getInstance().pause)
        {
            ...
            //@30초 이전에 나오는 몬스터
            if (GM.GameManager.getInstance().start_time <= 30f && GM.GameManager.getInstance().start_time - GM.GameManager.getInstance().Now_time >= 0.8f)
            {
            ...
            //@70초 이전에 나오는 몬스터
            else if (GM.GameManager.getInstance().start_time <= 70f && GM.GameManager.getInstance().start_time - GM.GameManager.getInstance().Now_time >= 0.8f)
            {
                GM.GameManager.getInstance().Now_time = GM.GameManager.getInstance().start_time;
